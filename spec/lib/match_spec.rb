...
#	after(:all){ @db.delete_database database: 'temp' }
	...
	context 'Simple Match Query'  do
		...
		context "add and modify where-filter" do
			Given( :m1 ) { q1.match_statements[0].where c: 9 }
			Then {  expect( m1.compose ).to eq   "MATCH {class: test_query, as: test_queries, where:( a = 9 and b = 's' and c = 9)} RETURN test_queries" }
